...
http.createServer((req, res) => {
  ...
  if (u.pathname === '/payload-check') {
    ...
    return jsonResponse(res, { diag: payloadDiagnostics(), rowCount: rows?.length || 0, error });
  }
  if (u.pathname === '/preflight-results') {
    const offset = Math.max(0, Number(u.searchParams.get('offset') || 0));
    const limit = Math.min(250, Math.max(1, Number(u.searchParams.get('limit') || 100)));
    return jsonResponse(res, { total: preflight.length, offset, items: preflight.slice(offset, offset + limit) });
  }
