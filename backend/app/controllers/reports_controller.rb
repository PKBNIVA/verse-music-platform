class ReportsController < ApplicationController
  ...
  def create
    ...
    return render_error("#{invalid} must be text.", :unprocessable_entity, "INVALID_REPORT") if invalid
    missing = %i[entityType entityId reason].find { params[_1].blank? }
    return render_error("#{missing} is required.", :unprocessable_entity, "INVALID_REPORT") if missing
    too_long = FIELD_LIMITS.find { |key, limit| params[key].to_s.length > limit }&.first
    return render_error("#{too_long} is too long.", :unprocessable_entity, "INVALID_REPORT") if too_long
