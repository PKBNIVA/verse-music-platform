module Admin
  class ReviewsController < BaseController
    def index = render(json: { reviews: Review.includes(:author, employer: :profile).order(created_at: :desc).limit(500).map { _1.attributes.merge(authorName: _1.author.name, employerName: _1.employer.profile&.company_name || _1.employer.name) } })
    def update
