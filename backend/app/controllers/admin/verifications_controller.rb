module Admin
  class VerificationsController < BaseController
    def index = render(json: { requests: VerificationRequest.includes(user: :profile).order(created_at: :desc).map { _1.attributes.merge(name: _1.user.name, email: _1.user.email, role: _1.user.role, companyName: _1.user.profile&.company_name) } })
    def update
