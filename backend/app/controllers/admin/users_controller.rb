module Admin
  class UsersController < BaseController
    ...
    def lookup
      ...
      return render(json: { email:, exists: false, emailProviderConfigured: email_provider, recentFailedLogins: failed_logins, diagnosis: [{ level: "error", code: "NO_ACCOUNT", message: "No account uses this email. Check the spelling, other addresses they may have used, or ask them to register." }] }) unless user
      ...
      facts = { status: user.status, email_verified: user.email_verified?, password_set: user.password_digest.present?, last_login_at: user.last_login_at,
                active_sessions:, recent_sessions:, tokens:, email_provider:, failed_logins: }
      ...
      render json: {
        ...
        emailTokens: tokens, recentAuthEvents: events, recentFailedLogins: failed_logins,
