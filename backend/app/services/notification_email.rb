...
class NotificationEmail
  ...
  # Only verified addresses are emailed: an unverified address may not belong to the account holder.
  def self.deliverable_to?(user)
    EmailDelivery.configured? && user.email.present? && user.email_verified? && user.status == "active"
  end

  # Returns { subject:, html:, text: }. Raises KeyError for an unknown template.
  def self.render(template, params, user)
    ...
    { subject:, html: html(heading:, copy:, action: spec[:action], link:), text: "#{heading}\n\n#{copy}\n\n#{link}" }
  end
