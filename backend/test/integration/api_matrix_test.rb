...
class ApiMatrixTest < ActionDispatch::IntegrationTest
  ...
  SPECS = [
    ...
    [:post, "/api/jobs", :talent, { ok: [201], params: job_params, bad: { title: "x", status: "draft" }, bad_status: [422], keys: %w[id status moderationFlags] }],
    ...
    [:post, "/api/job-alerts", :jobseeker, { ok: [201], params: { name: "Nightly", frequency: "daily" }, bad: { frequency: "hourly" }, bad_status: [400] }],
    [:patch, "/api/job-alerts/{alert}", :jobseeker, { params: { active: false }, idor: true, missing: :alert, bad: { frequency: "hourly" }, bad_status: [400], keys: %w[alert] }],
    ...
    [:patch, "/api/employer/jobs/{job}", :talent, { params: { status: "closed" }, idor: true, missing: :job, bad: { status: "published" }, bad_status: [400] }],
    [:put, "/api/employer/jobs/{job}", :talent, { params: { status: "draft" }, idor: true, missing: :job }],
    ...
    [:post, "/api/bookings", :talent, { ok: [201], params: ->(w, a) { { actId: w.refs[a == :js ? :js2 : :emp2][:act], eventType: "wedding", city: "Pune" } }, bad: {}, bad_status: [404, 422] }],
    ...
    [:post, "/api/bookings/{requested_booking}/status", :talent, { params: { status: "cancelled" }, idor: true, missing: :requested_booking, bad: { status: "bogus" }, bad_status: [409] }],
