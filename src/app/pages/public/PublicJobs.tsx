...
export default function PublicJobs(){
  usePageMeta('Music jobs, gigs, sessions & auditions','Browse open music jobs, gigs, studio sessions, auditions and tours across performance, production and live events.');
  const[sp,setSp]=useSearchParams();
  const[jobs,setJobs]=useState<any[]>([]),[q,setQ]=useState(sp.get('q')||''),[location,setLocation]=useState(sp.get('location')||''),[kind,setKind]=useState(()=>{const k=sp.get('kind')||'';return k&&!k.endsWith('s')?`${k}s`:k});
  ...
  return <div className="min-h-screen bg-slate-950 text-white"><PublicNav/><main className="max-w-6xl mx-auto px-5 py-14">
    ...
    <div className="flex flex-wrap gap-2 mt-6" role="group" aria-label="Opportunity type"><Button size="sm" aria-pressed={!kind} variant={!kind?'secondary':'outline'} onClick={()=>chooseKind('')}>All</Button>{kinds.map(x=><Button key={x} size="sm" aria-pressed={kind===x} variant={kind===x?'secondary':'outline'} onClick={()=>chooseKind(x)} className="capitalize">{x}</Button>)}</div>
