...
const EMPTY:Data={stats:{},users:[],jobs:[],reviews:[],verifications:[],reports:[],logs:[],subscriptions:[],bookings:[],attempts:[],billingEvents:[]};
...
export default function AdminDashboard(){
  ...
  const{stats,users,jobs,reviews,verifications,reports,logs,subscriptions,bookings,attempts,billingEvents}=data;
  ...
  return <div className="min-h-screen bg-slate-950 text-white">
    ...
    <main className="max-w-[1500px] mx-auto px-4 md:px-6 py-8">
      ...
      <TabsContent value="commerce" className="mt-5 grid xl:grid-cols-2 gap-5">
        ...
        <Card className="bg-white/[.05] border-white/10 xl:col-span-2"><CardHeader><CardTitle><h2>Billing events</h2></CardTitle><p className="text-sm text-slate-400">Razorpay webhooks as they were processed (read-only, newest first).</p></CardHeader><CardContent className="space-y-3 max-h-[520px] overflow-auto focus-visible:outline focus-visible:outline-2 focus-visible:outline-violet-400" tabIndex={0} role="region" aria-label="Billing events"><Panel error={errors.billingEvents} onRetry={retry} loading={loading}>{billingEvents.map(e=><div key={e.id} className="border-b border-white/10 pb-3 flex flex-col md:flex-row md:items-center justify-between gap-2"><div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><b className="break-all">{e.eventType}</b><Badge className={/fail|error|reject/i.test(e.processingResult||'')?'bg-rose-500/15 text-rose-300':'bg-white/10 text-slate-200'}>{e.processingResult||'received'}</Badge></div><div className="text-xs text-slate-400 mt-1 break-all">{[e.email,e.subscriptionId,e.paymentId,e.orderId].filter(Boolean).join(' · ')||e.providerEventId}</div></div><div className="text-xs text-slate-400 shrink-0">{e.amount!=null?`${(e.currency||'INR').toUpperCase()} ${(Number(e.amount)/100).toLocaleString()} · `:''}{date(e.processedAt||e.createdAt,true)}</div></div>)}{!billingEvents.length&&<Empty text="No billing events received."/>}</Panel></CardContent></Card>
