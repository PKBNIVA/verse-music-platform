...
export function ProductTour({role='public',forceOpen=false,onClose}:{role?:Role;forceOpen?:boolean;onClose?:()=>void}){
  ...
  useEffect(()=>{if(forceOpen||(!localStorage.getItem(key)&&role!=='public'))setOpen(true)},[forceOpen,key,role]);
  const close=()=>{localStorage.setItem(key,'done');setOpen(false);onClose?.()};
