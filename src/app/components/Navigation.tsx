...
export function Navigation() {
  ...
  const refreshUnread = () => apiGet<any>('/notifications/unread').then(d => { setUnread(Number(d.unread) || 0); setUnreadMessages(Number(d.unreadMessages) || 0); }).catch(() => {});
  useEffect(() => { void refreshUnread(); }, [location.pathname]);
  useEffect(() => { const onChange = () => { void refreshUnread(); }; window.addEventListener(UNREAD_CHANGED_EVENT, onChange); return () => window.removeEventListener(UNREAD_CHANGED_EVENT, onChange); }, []);
  useVisiblePolling(refreshUnread, UNREAD_POLL_MS, Boolean(user));
  const messageBadge = (className:string) => unreadMessages>0 ? <span data-testid="unread-messages-badge" className={`grid h-4 min-w-4 place-items-center rounded-full bg-fuchsia-500 px-1 text-[9px] font-bold text-white ${className}`} aria-hidden="true">{unreadMessages>9?'9+':unreadMessages}</span> : null;
    ...
    <nav className="fixed top-0 z-50 w-full border-b border-white/10 bg-[#070813]/88 backdrop-blur-2xl" aria-label="Workspace navigation">
      <div className="mx-auto flex h-[72px] max-w-[1500px] items-center gap-4 px-4 md:px-6">
        ...
        <div className="ml-5 hidden items-center gap-1 lg:flex">
          ...
          <Button variant="ghost" size="sm" asChild className={active(`${baseUrl}/messages`)?'bg-white/10 text-white':'text-slate-300'}><Link to={`${baseUrl}/messages`} aria-label={messagesLabel}><MessageSquare size={16} className="mr-2" />Messages{messageBadge('ml-1.5')}</Link></Button>
        </div>
        <div className="ml-auto flex items-center gap-1">
          <Button variant="ghost" size="icon" asChild className="relative text-slate-300 hover:text-white"><Link to={`${baseUrl}/notifications`} aria-label={unread>0?`Notifications, ${unread} unread`:'Notifications'}><Bell size={19} aria-hidden="true" />{unread>0&&<span data-testid="unread-notifications-badge" aria-hidden="true" className="absolute right-0.5 top-0.5 grid h-4 min-w-4 place-items-center rounded-full bg-fuchsia-500 px-1 text-[9px] font-bold text-white">{unread>9?'9+':unread}</span>}</Link></Button>
          ...
          <DropdownMenu><DropdownMenuTrigger asChild><Button variant="ghost" size="icon" className="lg:hidden" aria-label="Open all workspace tools"><Menu size={20}/></Button></DropdownMenuTrigger><DropdownMenuContent align="end" className="max-h-[72vh] w-72 overflow-y-auto">{groups.map(group => <div key={group.label}><DropdownMenuLabel className="text-[10px] uppercase tracking-widest">{group.label}</DropdownMenuLabel>{group.items.map(item=>{const Icon=item.icon;return <DropdownMenuItem key={item.path} asChild><Link to={item.path}><Icon className="mr-2 h-4 w-4"/>{item.label}</Link></DropdownMenuItem>})}<DropdownMenuSeparator/></div>)}<DropdownMenuItem asChild><Link to={`${baseUrl}/messages`}><MessageSquare className="mr-2 h-4 w-4"/>Messages</Link></DropdownMenuItem></DropdownMenuContent></DropdownMenu>
        </div>
      </div>
    </nav>
    <nav className="fixed inset-x-3 bottom-3 z-50 grid grid-cols-4 rounded-2xl border border-white/15 bg-[#101221]/94 p-1.5 shadow-2xl backdrop-blur-2xl lg:hidden" aria-label="Quick navigation">{quick.map(item=>{const Icon=item.icon;return <Link key={item.path} to={item.path} aria-label={item.path===`${baseUrl}/messages`&&unreadMessages>0?`${item.label}, ${unreadMessages} unread`:undefined} className={`relative flex min-h-12 flex-col items-center justify-center gap-1 rounded-xl text-[10px] font-semibold ${active(item.path)||location.pathname===item.path?'bg-violet-500/20 text-violet-200':'text-slate-400'}`}><Icon size={18}/>{item.label}{item.path===`${baseUrl}/messages`&&messageBadge('absolute right-[22%] top-1')}</Link>})}</nav>
  </>;
}
